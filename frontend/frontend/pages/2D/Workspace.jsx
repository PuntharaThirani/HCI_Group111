...
const Workspace = () => {
  ...
  // --- HELPER: Save to History before any change ---
  const saveToHistory = () => {
    setHistory((prev) => [...prev, [...items]].slice(-20)); // Keep last 20 steps
  };
  ...
  const addFurniture = (furniture) => {
    ...
    const newItem = { 
      ...
      id: Date.now(), 
      rotation: 0,
      x: roomData.width / 2, // Drop in center
      y: roomData.length / 2 
    };
  ...
  const updateItemPosition = (id, newX, newY) => {
    // Note: We don't save every pixel move to history to avoid lag
    setItems(items.map(item => item.id === id ? { ...item, x: newX, y: newY } : item));
  };
  ...
  const saveDesign = async () => {
    ...
    else alert("Design saved successfully! 🚀");
  };
      ...
      <nav className="flex items-center justify-between px-8 py-3 bg-white border-b border-slate-200 z-30 shadow-sm">
        <div className="flex items-center gap-6">
          ...
          <div className="flex flex-col">
            <span className="text-[9px] font-black text-orange-600 uppercase tracking-[0.2em] leading-none mb-1">Lead Architect</span>
            <span className="text-[11px] font-bold text-slate-700 tracking-tight">Punthara Thirani (Member 03)</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
             <div className="text-right">
               <p className="text-[10px] font-black text-slate-800 leading-none">Group 111</p>
             ...
             <div className="w-10 h-10 rounded-full border-2 border-orange-100 p-0.5 shadow-sm">
               <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Punthara" alt="profile" className="rounded-full bg-slate-100" />
             </div>
          </div>
        </div>
      </nav>
        ...
        <aside className="w-80 bg-white border-l border-slate-200 flex flex-col shadow-2xl z-20">
          ...
          <div className="p-8 h-[35%] overflow-y-auto bg-slate-50/30">
              ...
              {selectedItem ? (
                ...
                <div className="text-center py-10 opacity-20 italic text-[10px] uppercase tracking-widest">Select an item to edit</div>
              )}
          </div>
